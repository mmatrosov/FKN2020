...
private:
    ...
    bool CheckDiagonal1(size_t i, size_t j) const {
        size_t d1 = 0;
        while (d1 <= j && d1 <= i && Table[i-d1][j-d1] == Table[i][j])
            ++d1;
        size_t d2 = 0;
        while (j + d2 < N && i + d2 < N && Table[i+d2][j+d2] == Table[i][j])
            ++d2;
        return d1 + d2 > K;
    }
    ...
    bool CheckDiagonal2(size_t i, size_t j) const {
        size_t d1 = 0;
        while (j + d1 < N && d1 <= i && Table[i-d1][j+d1] == Table[i][j])
            ++d1;
        size_t d2 = 0;
        while (d2 <= j && i + d2 < N && Table[i+d2][j-d2] == Table[i][j])
            ++d2;
        return d1 + d2 > K;
    }
};
