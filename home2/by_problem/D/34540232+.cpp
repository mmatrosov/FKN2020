...
void dfs(node *curNode, std::string &curDirectory) {
  ...
  for (auto [name, child] : curNode->children) dfs(child, curDirectory);
  for (int i = 0; i <= curNode->name.length(); i++) curDirectory.pop_back();
}
...
int main() {
  node *root = new node();  //
  std::string path;
  while (std::cin >> path) addFile(path, root);
  path = "";
  dfs(root, path);
  return 0;
}